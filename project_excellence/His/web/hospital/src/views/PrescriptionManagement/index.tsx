...
export default function View() {
  ...
  const columns: ColumnsType<PrescriptionDataType> = [
    ...
    },
    {
      ...
      render: (_, record) => {
        return (
          <Tag color={record.prescriptionStatus == 1 ? "success" : "warning"}>{record.prescriptionStatus == 1 ? '已开具' : '未开具'}</Tag>
        )
      }
    },
    {
  ...
  return (
    <Card className={style.allPage} size='small' title="处方开具" extra={topBtn} bordered={false} style={{ width: '100%', minHeight: 'calc(100vh - 80px)' }}>
      {/* 搜索组件 */}
