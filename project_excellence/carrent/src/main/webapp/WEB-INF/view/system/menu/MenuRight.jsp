...
%>
        ...
        <script src="${alfred }/static/layui/layui.js"></script>
        ...
        ,url:'<%=path%>/menu/loadAllMenu.action' //数据接口
        ...
        alert(params);
        tableIns.reload({
        url:"<%=path%>/menu/loadAllMenu.action?"+params
        })
        });
        ...
        if(layEvent === 'del'){ //删除
        //先判断当前菜单有没有子节点
        $.post("<%=path%>/menu/checkMenuHasChildren.action?id="+data.id,function(obj){
        if(obj.code>=0){
        layer.msg("当前菜单有子节点，请先删除完子节点！");
        }else{
        layer.confirm('确定删除【'+data.title+'】这个菜单吗？', function(index){
        //向服务端发送删除指令
        $.post("<%=path%>/menu/deleteMenu.action",{id:data.id},function(res){
        layer.msg(res.msg);
        //刷新数据 表格
        tableIns.reload();
        //刷新左边的树
        ...
        //移除打开的样式
        $("#menuSelectDiv").removeClass("layui-show");
        url="<%=path%>/menu/updateMenu.action";
        //反选下拉树
        var pid=data.pid;
        ...
        url: "<%=path%>/menu/loadMenuManagerLeftTreeJson.action?spread=1", // 使用url加载（可与data加载同时存在）
        icon: "2",
        accordion:true
        });
        ...
        tableIns.reload({
        url:"<%=path%>/menu/loadAllMenu.action?id="+id
        })
        }
        </script>
        </body>
