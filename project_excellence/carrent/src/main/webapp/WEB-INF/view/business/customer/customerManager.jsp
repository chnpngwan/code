...
<%--        <div class="layui-input-inline">--%>
<%--            <input type="text" name="money" class="layui-input" maxlength="8" pattern="d{1,8}">--%>
...
<script type="text/javascript">
    ...
    layui.use(['jquery', 'layer', 'form', 'table'], function () {
        ...
        tableIns = table.render({
            ...
            , url: '<%=path%>/customer/loadAllCustomer.action' //数据接口
            ...
            , cols: [[   //列表数据
                ...
                , {field: 'money', title: '余额', align: 'center', width: '200'}
        ...
        $("#doSearch").click(function () {
            ...
            tableIns.reload({
                url: "<%=path%>/customer/loadAllCustomer.action?" + params,
                page: {curr: 1}
            })
        });
        ...
        $("#doExport").click(function () {
            var params = $("#searchFrm").serialize();
            window.location.href="<%=path%>/stat/exportCustomer.action?"+params;
        });
        ...
        table.on('tool(customerTable)', function (obj) {
            ...
            if (layEvent === 'del') { //删除
                layer.confirm('真的删除【' + data.custname + '】这个客户么？', function (index) {
                    //向服务端发送删除指令
                    $.post("<%=path%>/customer/deleteCustomer.action", {identity: data.identity}, function (res) {
                        layer.msg(res.msg);
                        //刷新数据表格
                        tableIns.reload();
                    })
        ...
        function openAddCustomer() {
            mainIndex = layer.open({
                ...
                success: function (index) {
                    //清空表单数据
                    $("#dataFrm")[0].reset();
                    url = "<%=path%>/customer/addCustomer.action";
                }
            });
        }
        ...
        function openUpdateCustomer(data) {
            mainIndex = layer.open({
                ...
                success: function (index) {
                    ...
                    url = "<%=path%>/customer/updateCustomer.action";
                }
            });
        }
        ...
        function deleteBatch() {
            ...
            layer.confirm('真的要删除这些客户么？', function (index) {
                //向服务端发送删除指令
                $.post("<%=path%>/customer/deleteBatchCustomer.action", params, function (res) {
                    layer.msg(res.msg);
                    //刷新数据表格
                    tableIns.reload();
                })
