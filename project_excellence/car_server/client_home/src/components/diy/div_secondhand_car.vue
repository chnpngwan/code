<template>
	<div class="diy_details_box diy_div_secondhand_car">
		<div class="warp">
			<div class="container">
					...
					<div class="view praise_wrap">
						...
						{{ praiseLen }}
							...
							<div class="details_btn_wrap">
								<button class="btn btn-primary details_btn" v-if="$check_action('/auction_order/edit','add')" @click="to_form('/auction_order/edit')" >竞拍 </button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/secondhand_car/details','set')" @click="change_praise()" >点赞</button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/secondhand_car/details','set')" @click="change_collect()" >收藏</button>
							</div>
						<!-- 富文本 -->
...
<script>
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "secondhand_car_id",
					source_id: this.obj["secondhand_car_id"],
					user_id
				};
              ...
              var praise_len = _this.praiseLen;
							this.$post('~/api/secondhand_car/set?secondhand_car_id=' + _this.obj["secondhand_car_id"], {
								praise_len
							}, (res) => {
								if(res.result){
      ...
      check_collect() {
        var user_id = this.$store.state.user.user_id;
        varsecondhand_car_id = this.obj.secondhand_car_id;
        this.$get('~/api/collect/count?', {
          user_id,
          source_table: "secondhand_car",
          source_field: "secondhand_car_id",
          source_id:secondhand_car_id
        }, (res) => {
