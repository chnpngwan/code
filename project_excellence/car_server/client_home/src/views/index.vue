		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/secondhand_car/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_secondhand_car = !show_list_secondhand_car">
		                        <div class="switch_box">
		                            <span v-if="show_list_secondhand_car"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--二手汽车推荐列表组件 -->
		                <list_secondhand_car v-if="show_list_secondhand_car" :list="list_secondhand_car" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_secondhand_car.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_secondhand_car" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_secondhand_car" :key="i" @click="to_details('secondhand_car',o,'secondhand_car_id')">
		                            <td v-for="(oj,n) in fields_secondhand_car">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
	...
	export default {
		...
		data() {
			return {
				...
				show_list_secondhand_car: true,
				list_secondhand_car: [],
				vm_arr_secondhand_car: [
					"secondhand_car_id",
					"auction_number",
					"seller_number",
					"seller_name",
		...
		methods: {
			...
			// 获取二手汽车列表
			get_secondhand_car() {
				let url = "~/api/secondhand_car/get_list?";
				...
				this.$get(url, {
					...
					if (json.result) {
						this.list_secondhand_car = json.result.list;
					}
				})
			},
		...
		computed: {
		    list_table_secondhand_car() {
		        var list = this.list_secondhand_car;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_secondhand_car.map((o) => {
		                // 第二个中括号是对象的属性
