						...
						<div class="card">
							...
							<div class="form_register">
								<b-form>
									...
									<!--买家-->
									<form_buyers v-if="form.user_group === '买家'" :form="form_sub"></form_buyers>
									<!--卖家-->
...
<script>
	...
	/* 买家 */
	import form_buyers from "@/components/diy/form_buyers";
	/* 卖家 */
			...
			async submit_check(param){
				...
				if(!ret && user_group){
					if(user_group === "买家"){
						...
						if(!ret && !form_sub["buyer_number"]){
							ret = "买家编号不能为空!";
						}
						else if(!ret){
							res = await this.$get("~/api/buyers/count?", {"buyer_number": form_sub["buyer_number"]});
							if(res.result){
								ret = "买家编号已存在!";
							}
						}
					...
					if(user_group === "卖家"){
						var form_sub = Object.assign({} ,this.form_sub);
						if(!ret && !form_sub["seller_number"]){
							ret = "卖家编号不能为空!";
						}
						else if(!ret){
							res = await this.$get("~/api/seller/count?", {"seller_number": form_sub["seller_number"]});
							if(res.result){
								ret = "卖家编号已存在!";
							}
						}
