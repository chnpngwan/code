<template>
	<div class="diy_details page_secondhand_car" id="secondhand_car_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_secondhand_car'>
							<div_secondhand_car :obj="obj"></div_secondhand_car>
...
<script>
	import div_secondhand_car from "@/components/diy/div_secondhand_car.vue";
	...
	export default {
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/secondhand_car/set?secondhand_car_id=' + obj["secondhand_car_id"], {
					hits
				}, (res) => {
					if(res.result){
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.secondhand_car_id;
          ...
          if (JSON.stringify(this.obj["timing_start_time"]).indexOf("-")===-1) {
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "secondhand_car",
					source_field: "secondhand_car_id",
					source_id: obj["secondhand_car_id"],
					user_id
				};
