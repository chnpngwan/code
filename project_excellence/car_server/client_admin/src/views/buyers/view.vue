		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','buyer_number') || $check_field('add','buyer_number') || $check_field('set','buyer_number')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="买家编号" prop="buyer_number">
					<el-input id="buyer_number" v-model="form_sub['buyer_number']" placeholder="请输入买家编号"
							  v-if="user_group === '管理员' || (form_sub['buyers_id'] && $check_field('set','buyer_number')) || (!form_sub['buyers_id'] && $check_field('add','buyer_number'))"></el-input>
					<div v-else-if="$check_field('get','buyer_number')">{{form_sub['buyer_number']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','buyer_name') || $check_field('add','buyer_name') || $check_field('set','buyer_name')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="买家姓名" prop="buyer_name">
					<el-input id="buyer_name" v-model="form_sub['buyer_name']" placeholder="请输入买家姓名"
							  v-if="user_group === '管理员' || (form_sub['buyers_id'] && $check_field('set','buyer_name')) || (!form_sub['buyers_id'] && $check_field('add','buyer_name'))"></el-input>
					<div v-else-if="$check_field('get','buyer_name')">{{form_sub['buyer_name']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','home_address') || $check_field('add','home_address') || $check_field('set','home_address')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="家庭住址" prop="home_address">
					<el-input id="home_address" v-model="form_sub['home_address']" placeholder="请输入家庭住址"
							  v-if="user_group === '管理员' || (form_sub['buyers_id'] && $check_field('set','home_address')) || (!form_sub['buyers_id'] && $check_field('add','home_address'))"></el-input>
					<div v-else-if="$check_field('get','home_address')">{{form_sub['home_address']}}</div>
				</el-form-item>
			</el-col>
...
<script>
	...
	export default {
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["buyers_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?buyers_id=" + form_sub["buyers_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/buyers/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/buyers/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/buyers/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/buyers/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/buyers/view','get');
