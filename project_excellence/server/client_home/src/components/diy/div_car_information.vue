<template>
	<div class="diy_details_box diy_div_car_information">
		<div class="warp">
			<div class="container">
					...
					<div class="view praise_wrap">
						...
						{{ praiseLen }}
							...
							<div class="details_btn_wrap">
								<button class="btn btn-primary details_btn" v-if="$check_action('/car_rental_order/edit','add')" @click="to_form('/car_rental_order/edit')" >租车 </button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/car_information/details','set')" @click="change_praise()" >点赞</button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/car_information/details','set')" @click="change_collect()" >收藏</button>
							</div>
						<!-- 富文本 -->
...
<script>
	...
	export default {
          ...
          if (res.result || res.result === 0) {
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					query.title = this.obj.automobile_brand
					query.title = this.obj.car_model
					query.img = this.obj.car_pictures
      ...
      check_collect() {
        var user_id = this.$store.state.user.user_id;
        varcar_information_id = this.obj.car_information_id;
        this.$get('~/api/collect/count?', {
          user_id,
          source_table: "car_information",
          source_field: "car_information_id",
          source_id:car_information_id
        }, (res) => {
