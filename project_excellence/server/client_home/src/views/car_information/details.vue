<template>
	<div class="diy_details page_car_information" id="car_information_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_car_information'>
							<div_car_information :obj="obj"></div_car_information>
						</div>
					</div>
				</div>
			</div>
...
<script>
	import div_car_information from "@/components/diy/div_car_information.vue";
	...
	export default {
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/car_information/set?car_information_id=' + obj["car_information_id"], {
					hits
				}, (res) => {
					if(res.result){
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.car_information_id;
				}
			},
			/**
			 * 初始化收藏状态
