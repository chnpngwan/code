						...
						<div class="card">
							...
							<div class="form_register">
								<b-form>
									...
									<!--会员用户-->
									<form_member_user v-if="form.user_group === '会员用户'" :form="form_sub"></form_member_user>
...
<script>
	...
	/* 会员用户 */
	import form_member_user from "@/components/diy/form_member_user";
				...
				if(!ret && user_group){
					if(user_group === "会员用户"){
						...
						if(!ret && !form_sub["user_name"]){
							...
							res = await this.$get("~/api/member_user/count?", {"user_name": form_sub["user_name"]});
							if(res.result){
								ret = "用户名已存在!";
							}
						}
