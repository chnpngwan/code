		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/car_information/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_car_information = !show_list_car_information">
		                        <div class="switch_box">
		                            <span v-if="show_list_car_information"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--汽车信息推荐列表组件 -->
		                <list_car_information v-if="show_list_car_information" :list="list_car_information" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_car_information.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_car_information" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_car_information" :key="i" @click="to_details('car_information',o,'car_information_id')">
		                            <td v-for="(oj,n) in fields_car_information">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
	...
	export default {
		...
		data() {
			return {
				...
				show_list_car_information: true,
				list_car_information: [],
				vm_arr_car_information: [
					"car_information_id",
					"car_number",
					"automobile_brand",
					"car_model",
		...
		methods: {
			...
			// 获取汽车信息列表
			get_car_information() {
				let url = "~/api/car_information/get_list?";
				this.$get(url, {
					"page": 1,
					"size": 8
				}, (json) => {
					if (json.result) {
						this.list_car_information = json.result.list;
					}
				})
			},
		...
		computed: {
		    list_table_car_information() {
		        var list = this.list_car_information;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_car_information.map((o) => {
		                // 第二个中括号是对象的属性
