	...
	<el-main class="bg edit_wrap">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			<el-col v-if="user_group === '管理员' || $check_field('get','user_name') || $check_field('add','user_name') || $check_field('set','user_name')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="用户名" prop="user_name">
						<div v-if="user_group !== '管理员'">
							...
							<!--v-if="user_group === '管理员' || (form['car_return_id'] && $check_field('set','user_name')) || (!form['car_return_id'] && $check_field('add','user_name'))" :disabled="disabledObj['user_name_isDisabled']"></el-input>-->
							<!--<div v-else-if="$check_field('get','user_name')">{{form['user_name']}}</div>-->
							<el-select v-if="user_group === '管理员' || (form['car_return_id'] && $check_field('set','user_name')) || (!form['car_return_id'] && $check_field('add','user_name'))" id="user_name" v-model="form['user_name']" :disabled="disabledObj['user_name_isDisabled']">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','full_name') || $check_field('add','full_name') || $check_field('set','full_name')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="姓名" prop="full_name">
					<el-input id="full_name" v-model="form['full_name']" placeholder="请输入姓名"
							  v-if="user_group === '管理员' || (form['car_return_id'] && $check_field('set','full_name')) || (!form['car_return_id'] && $check_field('add','full_name'))" :disabled="disabledObj['full_name_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','contact_information') || $check_field('add','contact_information') || $check_field('set','contact_information')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="联系方式" prop="contact_information">
					<el-input id="contact_information" v-model="form['contact_information']" placeholder="请输入联系方式"
							  v-if="user_group === '管理员' || (form['car_return_id'] && $check_field('set','contact_information')) || (!form['car_return_id'] && $check_field('add','contact_information'))" :disabled="disabledObj['contact_information_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','car_number') || $check_field('add','car_number') || $check_field('set','car_number')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="汽车编号" prop="car_number">
					<el-input id="car_number" v-model="form['car_number']" placeholder="请输入汽车编号"
							  v-if="user_group === '管理员' || (form['car_return_id'] && $check_field('set','car_number')) || (!form['car_return_id'] && $check_field('add','car_number'))" :disabled="disabledObj['car_number_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','rental_price') || $check_field('add','rental_price') || $check_field('set','rental_price')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="租赁价格" prop="rental_price">
					<el-input id="rental_price" v-model="form['rental_price']" placeholder="请输入租赁价格"
							  v-if="user_group === '管理员' || (form['car_return_id'] && $check_field('set','rental_price')) || (!form['car_return_id'] && $check_field('add','rental_price'))" :disabled="disabledObj['rental_price_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','lease_duration') || $check_field('add','lease_duration') || $check_field('set','lease_duration')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="租赁时长" prop="lease_duration">
					<el-input id="lease_duration" v-model="form['lease_duration']" placeholder="请输入租赁时长"
							  v-if="user_group === '管理员' || (form['car_return_id'] && $check_field('set','lease_duration')) || (!form['car_return_id'] && $check_field('add','lease_duration'))" :disabled="disabledObj['lease_duration_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','lease_duration')">{{form['lease_duration']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','renewal_duration') || $check_field('add','renewal_duration') || $check_field('set','renewal_duration')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="续租时长" prop="renewal_duration">
					<el-input id="renewal_duration" v-model="form['renewal_duration']" placeholder="请输入续租时长"
							  v-if="user_group === '管理员' || (form['car_return_id'] && $check_field('set','renewal_duration')) || (!form['car_return_id'] && $check_field('add','renewal_duration'))" :disabled="disabledObj['renewal_duration_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','renewal_duration')">{{form['renewal_duration']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','renewal_date') || $check_field('add','renewal_date') || $check_field('set','renewal_date')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="续租日期" prop="renewal_date">
					<el-date-picker :disabled="disabledObj['renewal_date_isDisabled']" v-if="user_group === '管理员' || (form['car_return_id'] && $check_field('set','renewal_date')) || (!form['car_return_id'] && $check_field('add','renewal_date'))" id="renewal_date"
						v-model="form['renewal_date']" type="date" placeholder="选择日期">
					</el-date-picker>
					<div v-else-if="$check_field('get','renewal_date')">{{form['renewal_date']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','return_status') || $check_field('add','return_status') || $check_field('set','return_status')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="还车状态" prop="return_status">
					<el-select id="return_status" v-model="form['return_status']"
						v-if="user_group === '管理员' || (form['car_return_id'] && $check_field('set','return_status')) || (!form['car_return_id'] && $check_field('add','return_status'))">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','remarks') || $check_field('add','remarks') || $check_field('set','remarks')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="备注" prop="remarks">
					<el-input id="remarks" v-model="form['remarks']" placeholder="请输入备注"
							  v-if="user_group === '管理员' || (form['car_return_id'] && $check_field('set','remarks')) || (!form['car_return_id'] && $check_field('add','remarks'))" :disabled="disabledObj['remarks_isDisabled']"></el-input>
...
<script>
	...
	export default {
		...
		data() {
			return {
				field: "car_return_id",
				url_add: "~/api/car_return/add?",
				url_set: "~/api/car_return/set?",
				url_get_obj: "~/api/car_return/get_obj?",
				url_upload: "~/api/car_return/upload?",
				...
				disabledObj:{
					...
					"renewal_duration_isDisabled": false,
					"renewal_date_isDisabled": false,
        ...
        if (this.form["renewal_date"].indexOf("-")===-1){
          this.form["renewal_date"] = this.$toTime(parseInt(this.form["renewal_date"]),"yyyy-MM-dd")
        }
				$.db.del("form");
				return param;
			},
        ...
        if(this.form["renewal_date"]=="0000-00-00"){
          this.form["renewal_date"] = null;
        }
				if(parseInt(this.form["renewal_date"]) > 9999){
					this.form["renewal_date"] = this.$toTime(parseInt(this.form["renewal_date"]),"yyyy-MM-dd")
				}
			},
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/car_return/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/car_return/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/car_return/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/car_return/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/car_return/view','get');
