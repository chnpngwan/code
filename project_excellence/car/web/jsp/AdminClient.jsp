			...
			<div style="position: initial;width: 100%;">
				<div align="center" id="header_div" class="grid" style="background-color: #000000;height:80px;line-height:80px;">
					...
					<li><img src="static/img/logo.png" style="width: 90%;" /></li>
					    <li><h1 style="color:white;">客户案例数据资源管理系统</h1></li>
						<!-- 管理员登录 -->
					</ul>
					<ul class="nav-pills">
						<li>
				...
				<div style="width: 1535px;margin: 0px auto;background-color: white;height: 620px;">
					<div class="col-md-8 column">
						<table style="text-align: center;" align="center" width="1180px" border="1" cellpadding="0" cellspacing="0">
						        ...
						        <c:forEach var="clienAdmintList" items="${clienAdmintList }">
						           <tr>
						              <td>${clienAdmintList.id }</td>
						              <td id="img_td">
						                 <img id="img_car" src="${clienAdmintList.img }"/>
						              </td>
						              <td>${clienAdmintList.name }</td>
						              <td>
						         ...
						         <div style="width: 1180px;margin: 0px auto;" class="grid">
									     <div align="center" style="position: absolute;margin-top: 330px;">
												当前第 ${pageInfo.pageNum } 页，共 ${pageInfo.pages } 页 共${clienAdmintList.size()}条记录<br/>
												<a href="<%=basePath %>/selectAllAdminClient.do?page=1" style="color:blue;">首页</a>&emsp;
												<a href="<%=basePath %>/selectAllAdminClient.do?page=${pageInfo.pageNum-1 }" style="color:blue;">上一页</a>&emsp;
												<c:forEach items="${pageInfo.navigatepageNums }" var="p">
												    <a href="<%=basePath %>/selectAllAdminClient.do?page=${p }"  style="color:blue;">${p }</a>&nbsp;
												</c:forEach>
												<a href="<%=basePath %>/selectAllAdminClient.do?page=${pageInfo.pageNum+1 }" style="color:blue;">下一页</a>&emsp;
												<a href="<%=basePath %>/selectAllAdminClient.do?page=${pageInfo.pages }" style="color:blue;">尾页</a>&emsp;
								         </div>  
									</div>
						</table> 
					</div>
