<template>
  <div>
    <div class="main">
      <div class="board" v-for="(item, index) in reputationList" :key="item.id">
        <div class="left">
...
export default {
  ...
  methods: {
    ...
    getReputationList() {
      this.axios.get("/api/rank/reputation").then(({ data }) => {
        if (data.code === 200) {
          this.reputationList = data.data.recordList;
          this.reputationList.forEach((movie) => {
