<template>
  <div>
    <div class="main">
      <div class="board" v-for="(item, index) in grossList" :key="item.id">
        <div class="left">
...
export default {
  ...
  methods: {
    ...
    getGrossList() {
      this.axios.get("/api/rank/gross").then(({ data }) => {
        if (data.code === 200) {
          this.grossList = data.data.recordList;
          this.grossList.forEach((movie) => {
