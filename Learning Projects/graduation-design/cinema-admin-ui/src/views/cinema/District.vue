<template>
  <el-card class="main-card">
    ...
    >
      ...
      <el-table-column prop="name" label="行政区域" />
  ...
  methods: {
    ...
    openDialog(object) {
      if (object != null) {
        ...
        this.$refs.title.innerHTML = "修改行政区域";
      } else {
        this.form = {};
        this.$refs.title.innerHTML = "添加行政区域";
      }
      this.dialog = true;
    },
    getList() {
    ...
    saveOrUpdate() {
      this.axios.post("/api/district", this.form).then(({ data }) => {
        if (data.code === 200) {
          this.$notify.success({
            title: "成功",
            message: data.message
    ...
    del(id) {
      ...
      this.axios.delete("/api/district", param).then(({ data }) => {
        if (data.code === 200) {
          this.$notify.success({
            title: "成功",
            message: data.message
