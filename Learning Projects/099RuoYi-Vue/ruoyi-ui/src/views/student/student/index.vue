    ...
    <el-table v-loading="loading" :data="studentList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="爱好" align="center" prop="studentHobby">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.hobby" :value="scope.row.studentHobby ? scope.row.studentHobby.split(',') : []"/>
        </template>
      </el-table-column>
...
export default {
  name: "Student",
  dicts: ['sys_yes_no', 'hobby', 'sys_user_sex'],
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      ...
      if (null != this.daterangeStudentBirthday && '' != this.daterangeStudentBirthday) {
        this.queryParams.params["beginStudentBirthday"] = this.daterangeStudentBirthday[0];
        this.queryParams.params["endStudentBirthday"] = this.daterangeStudentBirthday[1];
      }
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除学生列表编号为"' + studentIds + '"的数据项？').then(function() {
