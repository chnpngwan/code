...
public class RequestToObject {
	public static <T> T getParamesToObject(Class<T> cls, HttpServletRequest request){
		...
		try {
			...
			//获取请求中的参数名︰即表单控件中 name属性的属性值或ajaxd定义的键枚举
			...
			while( names.hasMoreElements()) {
				...
				methodName = "set"+ name.substring(0, 1).toUpperCase() + name.substring(1);// sexXxx严格遵循大小写
				
				method = setters.getOrDefault( methodName, null);//根据方法名查询对应的方法﹑无对应则返回null
				if( null == method){//如果没有对应的方法则说明这个类中属性不仅需要注值则跳过不管
					continue;
				}
				
				types = method.getParameterTypes();//有对应的方法则获取这个方法的形参的参数类型列表
				if( null == types || types.length <= 0){//说明没有形参也无法注值
					continue;
				}

				type = types[0]; //set方法默认只有一个形参所以取第一个形参的类型
				value = request.getParameter(name);//获取请求中的参数
			
				//类型匹配开启注值
				if( Integer.TYPE == type){//说明要的是一个整型值
				method.invoke( t, Integer.parseInt( value ));//反向激活﹐对象参数值
				}else if( Integer.class == type){  //整型对象类型可以设置null Integer包装类
